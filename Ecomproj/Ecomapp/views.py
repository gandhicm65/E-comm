...
from Ecomapp.form import CustomUserForm
...
def collections(request):
    ...
    return render(request, 'collections.html', {'cate':cat})
def collectionsview(request,name):
    if(Category.objects.filter(name=name,status=0)):
        product = Product.objects.filter(Category__name=name)
        return render(request, 'collectview.html', {'products':product,'category_name':name})
    else:
